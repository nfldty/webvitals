...
const Overview = () => {
  ...
  // Data Fetching
  const fetchStats = async () => { setLoading(true); setError(null); try { const activeFilters = Object.entries(filters).filter(([, value]) => value && typeof value === 'string' && value.trim() !== '').reduce((acc, [key, value]) => { acc[key] = value.trim(); return acc; }, {}); const isFiltering = Object.keys(activeFilters).length > 0; const endpoint = '/statistics'; const params = { userId: effectiveUserId, ...activeFilters }; const response = await api.get(endpoint, { params }); setStats(response.data || null); } catch (err) { console.error('Error fetching statistics:', err); const errorMessage = err.response?.data?.message || err.message || 'Failed to load statistics.'; setError(errorMessage); setStats(null); } finally { setLoading(false); } };
  // Filter Handling
